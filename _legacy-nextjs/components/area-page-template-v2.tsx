...
import { DomainConfig, getCityAreas } from '@/lib/domains-config';
import { generateAreaPageUniqueContent, AreaPageUniqueContent } from '@/lib/area-unique-content';
import { BookingForm, WhatsAppFloatWithForm, SidebarBookingForm } from '@/components/booking-form';

interface AreaPageTemplateV2Props {
  domain: DomainConfig;
  areaName: string;
  areaSlug: string;
}
...
export function AreaPageTemplateV2({ domain, areaName, areaSlug }: AreaPageTemplateV2Props) {
  const content = generateAreaPageUniqueContent(domain, areaName, areaSlug);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="bg-muted/30 py-3 border-b">
        <div className="container mx-auto px-4">
          <nav className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <ChevronRight className="h-4 w-4" />
            <span style={{ color: domain.colors.primary }}>{areaName}</span>
          </nav>
        </div>
      </div>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div>
              <Badge className="mb-4 bg-white/20 text-white border-white/30">
                <MapPin className="h-3 w-3 mr-1" /> {areaName}, {cityName}
              </Badge>
              ...
              <div className="flex flex-wrap gap-4 mb-8">
                <span className="flex items-center gap-2 bg-white/10 px-3 py-2 rounded-full text-sm">
                  <Check className="h-4 w-4" /> Free Delivery in {areaName}
                </span>
                <span className="flex items-center gap-2 bg-white/10 px-3 py-2 rounded-full text-sm">
                  <Check className="h-4 w-4" /> Same Day Setup
                </span>
                <span className="flex items-center gap-2 bg-white/10 px-3 py-2 rounded-full text-sm">
                  <Check className="h-4 w-4" /> Local Team
                </span>
              </div>
            ...
            <div className="hidden lg:block">
              <BookingForm domain={domain} pageTitle={`Services in ${areaName}`} variant="hero" />
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4">
          <div className="flex flex-wrap justify-center items-center gap-8 text-muted-foreground">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm">1000+ Celebrations in {areaName}</span>
            </div>
            <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2">
              <Clock className="h-4 w-4" style={{ color: domain.colors.primary }} />
      ...
      <section className="py-16">
        <div className="container mx-auto px-4">
          <div className="grid lg:grid-cols-3 gap-12">
            ...
            {/* Sidebar - 1/3 width */}
            <div className="hidden lg:block">
              <SidebarBookingForm domain={domain} pageTitle={`Services in ${areaName}`} />
                ...
                <CardContent>
                  <div className="flex flex-wrap gap-2">
                    ...
                    {otherAreas.length > 15 && (
                      <span className="text-xs text-muted-foreground">+{otherAreas.length - 15} more</span>
                    )}
                  </div>
                </CardContent>
              </Card>
      ...
      <section className="py-12 bg-muted/30">
        <div className="container mx-auto px-4">
          <h2 className="text-2xl md:text-3xl font-bold mb-8 text-center" style={{ color: domain.colors.primary }}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {otherAreas.slice(0, 10).map((area) => (
              ...
              >
                <MapPin className="h-5 w-5 mx-auto mb-2" style={{ color: domain.colors.primary }} />
      ...
      <section className={`${gradientClass} text-white py-16`}>
        <div className="container mx-auto px-4 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a href={`https://wa.me/${domain.whatsapp}?text=Hi, I'm interested in celebrations in ${areaName}`}>
            ...
            <div>
              ...
              <ul className="space-y-2 text-sm text-muted-foreground">
                {areas.slice(0, 5).map((area) => (
                  <li key={area.slug}>
                    <Link href={`/${area.slug}`} className="hover:text-primary">{area.name}</Link>
                  </li>
                ))}
              </ul>
            </div>
      ...
      <WhatsAppFloatWithForm domain={domain} pageTitle={`Services in ${areaName}`} />
    </div>
  );
}

export default AreaPageTemplateV2;
