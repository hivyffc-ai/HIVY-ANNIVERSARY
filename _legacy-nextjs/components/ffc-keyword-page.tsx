...
export default function FFCKeywordPage({ service, keyword }: KeywordPageProps) {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-purple-50 py-4">
        <div className="container mx-auto px-4">
          <nav className="flex items-center gap-2 text-sm flex-wrap">
            ...
            <Link href="/services" className="text-gray-500 hover:text-purple-800">Services</Link>
            <ChevronRight className="h-4 w-4 text-gray-400" />
            <Link href={`/services/${service.slug}`} className="text-gray-500 hover:text-purple-800">{service.name}</Link>
            <ChevronRight className="h-4 w-4 text-gray-400" />
            <span className="text-purple-800 font-medium">{keyword.title}</span>
          </nav>
        </div>
      </div>

      {/* Hero Section */}
      <section className="bg-gradient-to-br from-purple-900 via-violet-800 to-indigo-900 text-white py-16 md:py-20">
        <div className="container mx-auto px-4">
            ...
            <div className="text-center lg:text-left">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
                <FFCBookNowButton 
                  ...
                  className="bg-white text-purple-800 hover:bg-purple-50 text-lg px-8 py-6" 
                />
                  ...
                  <div className="grid md:grid-cols-2 gap-3">
                    {[
                      ...
                      <div key={index} className="flex items-center gap-2">
                        <Check className="h-4 w-4 text-purple-800 flex-shrink-0" />
                  ...
                  {relatedPackages.map((pkg) => (
                    <Link key={pkg.id} href={`/packages/${pkg.slug}`}>
                      <Card className="h-full hover:shadow-lg transition-all hover:-translate-y-1 border-purple-100 group">
                        <div className="aspect-video bg-gradient-to-br from-purple-100 to-slate-50 relative overflow-hidden">
                          <Image
                            src={pkg.thumbnail}
                            alt={pkg.name}
                            fill
                        ...
                        <CardContent className="p-4">
                          <h3 className="font-semibold mb-1 group-hover:text-purple-800 transition-colors">
                            {pkg.name}
                          </h3>
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4 max-w-4xl mx-auto">
            {relatedKeywords.map((kw) => (
              ...
              >
                <Card className="border-purple-100 hover:border-purple-200 hover:shadow-md transition-all group">
                  <CardContent className="p-4">
                    <h3 className="font-medium group-hover:text-purple-800 transition-colors flex items-center justify-between">
                      {kw.title}
                      <ChevronRight className="h-4 w-4 text-gray-400 group-hover:text-purple-800" />
                    </h3>
                  </CardContent>
                </Card>
              </Link>
          ...
          <div className="flex flex-wrap justify-center gap-3 max-w-4xl mx-auto">
            {suratAreas.slice(0, 12).map((area) => (
              <Link 
                key={area.slug}
                href={`/${area.slug}`}
                className="px-4 py-2 bg-purple-50 rounded-full text-gray-700 hover:bg-purple-800 hover:text-white transition-colors border border-purple-200 text-sm"
              >
                {area.name}
              </Link>
            ))}
      ...
      {/* CTA Section */}
      <section className="py-16 bg-gradient-to-br from-purple-800 to-violet-700 text-white">
