      ...
      <div class="grid md:grid-cols-3 gap-6 my-6">
        ...
        <div class="border-2 border-primary/20 rounded-xl p-6 hover:border-primary transition-colors">
          ...
          <ul class="space-y-2 text-sm">
            ...
            <li class="flex items-center gap-2">✓ Private private venue/Pool Access</li>
...
function generateDetailedSection(domain: DomainConfig, keyword: KeywordConfig, keywordCount: { count: number }): ContentSection {
  ...
  if (keyword.slug.includes('birthday')) {
    ...
    specificContent = `
      ...
      <p class="mb-4">Experience the magic of dining under the open sky with our exclusive <strong>${kw}</strong> arrangements in ${city}. Our elegant venues offer breathtaking views of the city skyline, creating a stunning backdrop for your romantic evening.</p>
...
function generateImages(domain: DomainConfig, keyword: KeywordConfig): ImageData[] {
  ...
  else if (keyword.slug.includes('private venue')) imageCategory = 'private venue-dining';
